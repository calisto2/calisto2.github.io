...
public class GestionEmpleados1 {
   ...
   e.setEdad(44);
   e.setSueldo(34350.0);
   empleados [0] = e; //se añade el empleado al array
   e = new Empleado();
   e.setNom("Carlos");
   e.setEdad(31);
   e.setSueldo(24350.0);
   empleados [1] = e; //se añade el empleado al array
	...
	try {		
		   for (int i=0; i<n;i++) { // Solicitamos la entrada de los empleado   
			  ...
			  do {
			   ...
			   e.setEdad(edad);
			   e.setSueldo(sueldo);
			   empleados [i+2] = e; //se añade el empleado al array
			   indice = indice + 1;
		   }  // end for
		...
		while (salida) {
			...
			System.out.println("5 Datos del Empleado que cobra más ");
			System.out.println("6- Datos del empleado más viejos");
...
public static Empleado SueldoAlto() {
	...
	return mayor;
	//System.out.println( "El sueldo más alto es de : " + sueldoMasAlto +  e.toString()); 
}
